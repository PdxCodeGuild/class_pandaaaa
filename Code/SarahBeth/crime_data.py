...
for i in range(len(data)):
    for crime in crime_dict: 
        if crime in crimes_counted:
            crimes_counted[crime] +=1
        else: 
            crimes_counted[crime] = 1
...
for crime in crimes_counted:
    highest_crime = ''
    if 
    if crimes_counted[crime] > 0:
        highest_crime = crime
    if crimes_counted[crime] greater than highest_crime:
        highest_crime = crime
...
def load_data():
    n_lines = None
    with open('../Data/crime_incident_data_recent.csv', 'r') as f:
        lines = f.read().split('\n')
...
def most_common_crime_per_hour(data):
    ...
    data.sort(key=lambda d: d['report_time'])
    for hour, hourly_crimes in groupby(data, key=lambda d: d['report_time']):
...
for row in data:
    row['report_time'] = int(row['report_time'])//100


most_common_crime_per_hour(data)


#group_data(data, 'report_time')
