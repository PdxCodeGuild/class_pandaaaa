...
if __name__ == '__main__':
    ...
    aparser.add_argument('-m', "--metre", required=False, default=default_metre,
                         help='the metre as a string of zeroes and ones', metavar='metre')
    aparser.add_argument('-f', "--file", required=True,
                         help='the input file containing poetry to be scanned', metavar='filename')
    aparser.add_argument('-e', "--element", required=False,
                         help='the name of the XML element containing a line of verse', metavar='line element')
    aparser.add_argument('-t', "--text", required=False,
                         help='assume the input is plain text', action="store_true")
    aparser.add_argument('-d', "--debug", required=False,
                         help='print debugging info', action="store_true")
    ...
    for l_struct in lines:
        ...
        stress_pattern_string = "".join(
            stress_patterns)  # ignore word boundaries
        ...
        stress_patterns_poetic = stresspatterns.get_stress_patterns_poetic(
            words)
        stress_pattern_string_poetic = "".join(
            stress_patterns_poetic)  # ignore word boundaries
        stress_pattern_num_poetic = map(int, stress_pattern_string_poetic)
        scansion_pattern_poetic = get_scansion_pattern(
            stress_pattern_num_poetic, metre_num)

        pattern_string = "".join(str(d) for d in scansion_pattern)
        pattern_string_poetic = "".join(str(d)
                                        for d in scansion_pattern_poetic)
        if levenshtein(pattern_string, metre) < levenshtein(pattern_string_poetic, metre):
            pattern_string_final = pattern_string
            wildcard_pattern_string = "".join(
                str(d) for d in get_wildcard_pattern(stress_pattern_num))
        else:
            pattern_string_final = pattern_string_poetic
            wildcard_pattern_string = "".join(
                str(d) for d in get_wildcard_pattern(stress_pattern_num_poetic))
