    ...
    with open(file_path, 'r') as file:
        ...
        for line in file:
            ...
            for i in range(len(data_str)):
                if i == 0:
                    data_line[i] = datetime.datetime.strptime(i, '%d-%b-%Y')  # convert the string to a datetime
...
def find_longest_continuous_rain(data):
    ...
    for i in range(len(data)):
        if data[i][1] is None:
            continue
        if data[i][1] == 0: # change to != 0 to get the longest dry period
...
def main():
    data = parse_file('mt_tabor_rain.txt')
